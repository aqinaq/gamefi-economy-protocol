...
The detected issues are either informational, expected by design, or low-risk findings documented below.
...
**Reason:** The contract only needs `answer` and `updatedAt`.
...
**Reason:** The oracle intentionally compares `block.timestamp` with Chainlink `updatedAt` to reject stale prices.
...
Slither reported use of a low-level `.call` in `Treasury.withdrawETH()`.

**Reason:** `.call` is the recommended modern replacement for `.transfer`.

**Mitigation:** The function is restricted by `onlyOwner`. In deployment, ownership is transferred to the Timelock contract.
...
**Reason:** No critical state is updated after the call. The function is owner-only.

**Mitigation:** Treasury ownership is controlled by the Timelock in the governance setup.
...
**Reason:** This is intentional. `SumUtils` exists to compare Solidity and Yul gas usage.
...
**Reason:** These are style warnings only.
